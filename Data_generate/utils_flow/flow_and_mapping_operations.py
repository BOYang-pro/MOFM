...
from Data_generate.utils_flow.pixel_wise_mapping import remap_using_correspondence_map
...
def get_gt_correspondence_mask(flow):
    ...
    if isinstance(mapping, np.ndarray):
        if len(mapping.shape) == 4:
            ...
            mask_y = np.logical_and(mapping[1] >= 0, mapping[1] <= h - 1)
            mask = np.logical_and(mask_x, mask_y)
        mask = mask.astype(bool) if version.parse(torch.__version__) >= version.parse("1.1") else mask.astype(np.uint8)
    else:
        if len(mapping.shape) == 4:
            # shape is B,C,H,W
            b, _, h, w = mapping.shape
