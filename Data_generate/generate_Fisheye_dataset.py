...
if __name__ == "__main__":
    ...
    for dataset in dataset_name:
        #test_folder = os.path.join(image_root, dataset)
        test_folder = os.path.join(image_root)
        image_extensions = ('.jpg', '.png', '.jpeg', '.bmp')
        num_images = len([f for f in os.listdir(os.path.join(test_folder,'ir')) if f.lower().endswith(image_extensions)])
        if num_images == 0:
        ...
        if num_images <= 500:
            ...
            flow_name = f'./fisheye/flow/{num}.npy'
            mask_name="./fisheye/mask.png"
        else:
        ...
        scale=0.8
        test_out_folder1= os.path.join(save_root,dataset,'vi_warp')
        test_out_folder2 = os.path.join(save_root,dataset,'ir_warp')
        test_out_folder3= os.path.join(save_root,dataset,'vi')
        test_out_folder4 = os.path.join(save_root,dataset,'ir')
        ...
        for img_name in tqdm(os.listdir(os.path.join(test_folder, "ir"))):
            ...
            mask = (ToTensor()(mask) * (min_max[1] - min_max[0]) + min_max[0]).unsqueeze(0)
            vi=vi.cuda()
            ir=ir.cuda()
            mask=mask.cuda()
            ...
            vi_warp = resample_image(vi*mask, -flow_gt*scale)
            ir_warp = resample_image(ir*mask, -flow_gt*scale)
            ...
            vi_gt = tensor2fu(vi*mask, min_max=(-1, 1)).astype(np.uint8)
            img_save(vi_gt, img_name.split(sep='.')[0], test_out_folder3)
            ir_gt = tensor2fu(ir*mask, min_max=(-1, 1)).astype(np.uint8)
